...
def main():
    ...
    # Logging
    os.makedirs("checkpoints/checkpoints_diffusion", exist_ok=True)
    writer = SummaryWriter("checkpoints/checkpoints_diffusion/tensorboard")
    ...
    for epoch in range(args.epochs):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.state_dict(), "checkpoints/checkpoints_diffusion/best_model.pt")
            print("  → New best model saved!")
            
        if (epoch + 1) % 5 == 0:
            torch.save(model.state_dict(), f"checkpoints/checkpoints_diffusion/checkpoint_epoch{epoch+1}.pt")
    
    torch.save(model.state_dict(), "checkpoints/checkpoints_diffusion/final_model.pt")
    print("\nTraining complete!")
