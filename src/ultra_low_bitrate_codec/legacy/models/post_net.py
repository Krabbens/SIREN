...
class Snake(nn.Module):
    ...
    def forward(self, x):
        ...
        elif x.shape[-1] == self.alpha.shape[1]:
    ...
    def forward(self, x):
        ...
        if x.dim() == 2:
            ...
            beta = beta.squeeze(-1)
             ...
             # (B, T, C) case - only if T != C to avoid ambiguity, or assume last dim is C if matches
             alpha = alpha.permute(0, 2, 1)
...
class SnakePhase(nn.Module):
    ...
    def forward(self, x):
        ...
        return x + (1.0 / (beta + 1e-9)) * (torch.sin(alpha * x + phi) ** 2 - torch.sin(phi) ** 2)
