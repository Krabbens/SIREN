...
def main():
    ...
    with torch.no_grad():
        for i, (wav, fname) in enumerate(tqdm(dataloader)):
            ...
            # 1. Compute Complex STFT (Target)
            # (B, 1, T) -> (B, n_fft/2+1, T, 2)
            stft = torch.stft(
                wav, 
                n_fft=n_fft, 
            ...
            # (B, 513, T [Complex])
            # Stack Real and Imag: (B, 1026, T)
            real = stft.real
            imag = stft.imag
            
            # Concatenate along channel dimension
            target = torch.cat([real, imag], dim=1) # (B, 1026, T)
            ...
            # The reconstructor output 'feats' is exactly what goes into the vocoder
            ...
            # Cond needs to be (B, 512, T_cond) to match typical Conv1d structure, but let's check FlowHead
            # My FlowMatchingHead expects:
            # x: (B, 80, T)
            # cond: (B, 512, T)
            
            cond = cond.transpose(1, 2) # (B, 512, T_cond)
            
            # Match sizes via interpolation to be safe (BitNet upsampling might be slightly off due to rounding)
            ...
            torch.save({
                'target': target.cpu().squeeze(0), # (1026, T)
