...
def load_models(vocoder_checkpoint, device='cuda'):
    ...
    encoder_ckpt = torch.load('checkpoints_micro_encoder/best_model.pt', map_location=device)
    encoder.load_state_dict(encoder_ckpt['model_state_dict'])
    encoder.eval()
    
    # Adapter
    adapter = FeatureAdapter(in_dim=768, out_dim=512, upsample_factor=1).to(device)
    adapter_ckpt = torch.load('checkpoints_adapter/checkpoint_epoch9.pt', map_location=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocoder", type=str, default="checkpoints_bitnet_v2_antibanding/best_model.pt")
    parser.add_argument("--audio", type=str, default="jakubie_16k.wav")
    ...
    if not os.path.exists(args.vocoder):
        ...
        for ckpt in ["checkpoints_bitnet/best_model.pt", "checkpoints_bitnet_v2_antibanding/checkpoint_epoch10.pt"]:
